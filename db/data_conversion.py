...
def convert_schedule(pg_schedule: list) -> list:
    ...
    for item in pg_schedule:
        ...
        info = schedule_tools.forming_info_data(
            nt=item['nt'], ngroup=item["ngroup"])
                ...
                for day_lesson in sch['lessons']:
                    ...
                    if lesson['time'] == day_lesson['time'] \
                        ...
                        if lesson['aud'] == day_lesson['aud']:
                            if item['preps']:
                                day_lesson['prep'].append(
                                    item['preps'].strip().strip('.'))
                        elif lesson['prep'] == day_lesson['prep']:
                            day_lesson['aud'].append(
                                item['auditories_verbose'])
                        break
        ...
        if current_group != next_group or item_index == len(pg_schedule) - 1:
            ...
            if schedule:
                ...
                schedule_tools.sorting_lessons_in_a_day_by_time_and_ngroup(
                    schedule=schedule)
